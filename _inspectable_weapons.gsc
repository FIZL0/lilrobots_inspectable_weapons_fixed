/*
	...
	Fizlo: Fixes getting stuck inspecting among other things
*/
...
function wait_for_inspect()
{
	...
	for(;;)
	{
		if(self ReloadButtonPressed())
		{
			...
			wait(0.3); //must hold use for > half a second..
			...
			if( self ReloadButtonPressed() && can_inspect && !self isReloading() && !self isSwitchingWeapons() && !self IsUsingOffHand() && !self isThrowingGrenade() //make sure you're not reloading, using offhands OR throwing grenades
				&& !self IsSprinting() && !self IsSliding())  // FIZLO fixes: checks if sprinting, sliding
			{
				wait(0.3);
				if( !self IsSprinting() ) // still not sprinting?
				{
					self AllowSlide(0);
					self thread auto_interrupt();
